...
def main():
    ...
    for test_subject in subjects:
        ...
        for i in range(len(X_test_scaled)):
            ...
            inc_weights += lr * error * features
            inc_bias += lr * error
            updates_count += 1
            
            # 4. Replay buffer logic
            if label == 0:
                if len(buffer_0) < buffer_size_per_class:
                    buffer_0.append((features, label))
                else:
                    buffer_0[buffer_idx_0] = (features, label)
                buffer_idx_0 = (buffer_idx_0 + 1) % buffer_size_per_class
            else:
                if len(buffer_1) < buffer_size_per_class:
                    buffer_1.append((features, label))
                else:
                    buffer_1[buffer_idx_1] = (features, label)
                buffer_idx_1 = (buffer_idx_1 + 1) % buffer_size_per_class
            
            if updates_count % 5 == 0:
                # Replay buffer pass
                replay_buffer = buffer_0 + buffer_1
                for b_features, b_label in replay_buffer:
                    ...
                    inc_weights += lr * b_error * b_features
                    inc_bias += lr * b_error
